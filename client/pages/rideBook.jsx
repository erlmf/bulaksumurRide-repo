    ...
    if (pickupQuery && dropoffQuery) {
      try {
        ...
        if (
          ...
          !pickupCoords.some(isNaN) &&
          !dropoffCoords.some(isNaN)
        ) {
          ...
          // Get street names
          getStreetName({ lat: pLat, lng: pLng }).then(setPickupName);
          getStreetName({ lat: dLat, lng: dLng }).then(setDropoffName);
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-sm bg-gray-50 p-2 rounded">
                      {pickupName ? `${pickupName} (${pickup})` : pickup}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-gray-700 font-medium">Drop-off Location</span>
                    <span className="text-sm bg-gray-50 p-2 rounded">
                      {dropoffName ? `${dropoffName} (${dropoff})` : dropoff}
                    </span>
                  </div>
